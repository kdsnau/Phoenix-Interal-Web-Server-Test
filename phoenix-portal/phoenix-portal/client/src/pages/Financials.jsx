...
/* -----------------------------------------------------------------------
   Create / edit an RFQ (snapshot_entries)
   ----------------------------------------------------------------------- */
...
/* -----------------------------------------------------------------------
   ...
   ----------------------------------------------------------------------- */
function RfqsTable({ rfqs, canManage, onEdit, onDelete }) {
    if (rfqs.length === 0) return <div className="fin-empty">No RFQs yet.</div>;
    return (
        <div className="fin-table-wrap">
            <table className="fin-table">
                <thead>
                    <tr><th>#</th><th>Customer</th><th>RFQ</th><th>Status</th><th>Hours</th><th>Scheduled</th><th>Invoice #</th><th>Added</th>{canManage && <th></th>}</tr>
                </thead>
                <tbody>
                    {rfqs.map(e => (
                        <tr key={e.id}>
                            ...
                            {canManage && (
                                <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                                    <button className="btn btn-ghost" style={{ padding: '4px 8px', fontSize: 12 }} onClick={() => onEdit(e)}>Edit</button>
                                    <button className="btn btn-ghost" style={{ padding: '4px 8px', fontSize: 12, color: 'var(--red)' }} onClick={() => onDelete(e.id)}>Del</button>
                                </td>
                            )}
                        </tr>
                    ))}
    ...
    return (
        <>
            <div className="stats-grid" style={{ marginBottom: 20 }}>
                <div className="stat-card"><div className="stat-label">Total MRR</div><div className="stat-value" style={{ color: 'var(--accent)' }}>{money2(total)}</div></div>
                <div className="stat-card"><div className="stat-label">Paying Clients</div><div className="stat-value">{clients.length}</div></div>
                <div className="stat-card"><div className="stat-label">Annualized</div><div className="stat-value" style={{ color: 'var(--green)' }}>{money2(total * 12)}</div></div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 28, flexWrap: 'wrap', alignItems: 'center', marginBottom: 24 }}>
                <svg viewBox="0 0 160 160" width="180" height="180" style={{ flexShrink: 0 }}>
                    ...
                    <text x={CX} y={CY - 3} textAnchor="middle" style={{ fill: 'var(--text-hi)', fontSize: 15, fontWeight: 600 }}>{money2(total).replace('.00', '')}</text>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minWidth: 240 }}>
                    {segments.map(seg => (
                        <div key={seg.key} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <span className="fin-mono" style={{ color: 'var(--text-hi)' }}>{money2(seg.value)}</span>
                        ...
                        {clients.map((c, i) => {
                            ...
                            return (
                                <tr key={c.id}>
                                    ...
                                    <td className="fin-mono" style={{ textAlign: 'right', color: 'var(--text-hi)' }}>{money2(c.mrr)}</td>
                        ...
                        <tbody>
                            {list.map(c => (
                                <tr key={c.id} style={{ cursor: 'pointer' }} onClick={() => onOpen(c)}>
                                    ...
                                    <td className="fin-mono" style={{ textAlign: 'right', color: 'var(--accent)' }}>{Number(c.mrr) > 0 ? money0(c.mrr) : '—'}</td>
    ...
    return (
        <>
            ...
            <div className="stats-grid" style={{ marginBottom: 20 }}>
                ...
                <div className="stat-card"><div className="stat-label">Recurring (MRR)</div><div className="stat-value" style={{ color: 'var(--accent)' }}>{Number(client.mrr) > 0 ? money2(client.mrr) : '—'}</div></div>
            </div>
            ...
            <div className="fin-section-tabs" style={{ marginBottom: 14, alignItems: 'center' }}>
                ...
                {canManage && (tab === 'work_orders' || tab === 'rfqs') && (
                    <button className="btn btn-primary" style={{ marginLeft: 'auto', fontSize: 12 }}
                        onClick={() => setModal({ kind: tab === 'work_orders' ? 'wo' : 'rfq', entry: null })}>
                        + New {tab === 'work_orders' ? 'Work Order' : 'RFQ'}
                    </button>
                )}
            </div>
            ...
            {tab === 'rfqs'        && <RfqsTable rfqs={rfqs} canManage={canManage} onEdit={e => setModal({ kind: 'rfq', entry: e })} onDelete={deleteRfq} />}
            ...
            <div className="fin-page">
                ...
                {loading ? (
                    ...
                    <RfqsTable rfqs={rfqs} canManage={canManage}
                        onEdit={e => setModal({ kind: 'rfq', entry: e })} onDelete={deleteRfq} />
                ) : tab === 'expenses' ? (
